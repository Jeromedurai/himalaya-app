...
function Header() {
    ...
    const { openSidebar, closeSidebar, fetchSingleProduct } = useProductsContext()
    ...
    const {
        updateFilters, filters: { searchTerm, cateId },
    ...
    if (!isLoading && data) {
        pageContent = data.map(({ menuId, menuName, subMenu, category }: any) => {
            return (
                <>
                    {
                        !subMenu && <li className='max-lg:border-b max-lg:py-3 px-3'>
                            <a href=""
                                className='hover:text-blue-600 text-blue-600 block font-semibold transition-all'>{menuName}
                            </a>
                        </li>
                    }
                    {subMenu && <li className='group max-lg:border-b max-lg:py-3 px-3 relative'>
                        <span
                            className='hover:text-[#007bff] text-gray-600 text-[15px] font-bold lg:hover:fill-[#007bff] block'>{menuName}
                            <DrillDown />
                        </span>
                        <ul
                            ...
                            {category.map(({ categoryId, category, name, active }: any) => {
                                return (
                                    <li
                                        title={name}
                            ...
                            {/* <li className='border-b py-2 '><Link to=''
                                className='hover:text-[#007bff] text-gray-600 text-[15px] font-bold block'>Contact</Link></li>
                            <li className='border-b py-2 '><Link to=''
                                className='hover:text-[#007bff] text-gray-600 text-[15px] font-bold block'>Login</Link></li>
                            <li className='border-b py-2 '><Link to=''
                                className='hover:text-[#007bff] text-gray-600 text-[15px] font-bold block'>Sign up</Link></li>
                            <li className='border-b py-2 '><Link to=''
                                className='hover:text-[#007bff] text-gray-600 text-[15px] font-bold block'>Blog</Link></li> */}
                        </ul>
                    </li>
                    }
                    {/* <li className='group max-lg:border-b max-lg:py-3 px-3 relative'>
                        <Link to=''
                            className='hover:text-[#007bff] text-gray-600 text-[15px] font-bold lg:hover:fill-[#007bff] block'>Blog
                            <DrillDown />
                        </Link>
                        <ul
                            className='absolute shadow-lg bg-white space-y-3 lg:top-5 max-lg:top-8 -left-6 min-w-[250px] z-50 max-h-0 overflow-hidden group-hover:opacity-100 group-hover:max-h-[700px] px-6 group-hover:pb-4 group-hover:pt-6 transition-all duration-500'>
                            <li className='border-b py-2 '><Link to=''
                                className='hover:text-[#007bff] text-gray-600 text-[15px] font-bold block'>Foods</Link></li>
                            <li className='border-b py-2 '>
                                <Link to=''
                                    className='hover:text-[#007bff] text-gray-600 text-[15px] font-bold block'>Sale</Link>
                            </li>
                            <li className='border-b py-2 '><Link to=''
                                className='hover:text-[#007bff] text-gray-600 text-[15px] font-bold block'>Marketing</Link></li>
                            <li className='border-b py-2 '><Link to=''
                                className='hover:text-[#007bff] text-gray-600 text-[15px] font-bold block'>Investment</Link></li>
                        </ul>
                    </li>
                    <li className='max-lg:border-b max-lg:py-3 px-3'>
                        <Link to=''
                            className='hover:text-blue-600 block font-semibold transition-all'>Blog
                        </Link>
                    </li>
                    <li className='max-lg:border-b max-lg:py-3 px-3'>
                        <Link to=''
                            className='hover:text-blue-600 block font-semibold transition-all'>About
                        </Link>
                    </li> */}
                </>
            )
