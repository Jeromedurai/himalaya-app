...
export const ProductCard = ({ product, dispatchCart, CART_REDUCER_ACTIONS, inCart }: PropsType): ReactElement => {
  ...
  const buttonText = in_stock ? 'Add To Bag' : 'Out of Stock';
  return (
    <div className="m-3 max-w-sm bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
      ...
      <div className="p-5">
        ...
        <p className="flex justify-between items-center">
          ...
          {/* {inCart && <button onClick={onGoToCart} className="inline-flex items-center py-2 px-3 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800">Item In Cart<i className="ml-1 bi bi-cart-fill"></i></button>} */}
...
function areProductsEqual({ product: prevProduct, inCart: prevInCart }: PropsType, { product: nextProduct, inCart: nextInCart }: PropsType) {
  return (
    Object.keys(prevProduct).every(key => {
      return prevProduct[key as keyof productDataType] ===
        nextProduct[key as keyof productDataType]
